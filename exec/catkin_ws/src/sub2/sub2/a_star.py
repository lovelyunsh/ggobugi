...
class a_star(Node):
    ...
    def __init__(self):
        ...
        self.dCost = [1,1,1,1,1.4,1.4,1.4,1.4]
    ...
    def timer_callback(self):
        if self.is_map ==True and self.is_odom==True  :
            ...
            # start_x, start_y = start_grid_cell[0], start_grid_cell[1]
            # while self.grid[start_x][start_y] != 0:
            #     start_x = start_x + 1
            #     start_y = start_y + 1
            #     if start_x >= 350 or start_y >= 350:
            #         start_x = 0
            #         start_y = 0
            # start_grid_cell = [ start_x , start_y ]
            self.path = [[0 for col in range(self.GRIDSIZE)] for row in range(self.GRIDSIZE)]
            self.cost = np.array([[2147483647 for col in range(self.GRIDSIZE)] for row in range(self.GRIDSIZE)])
            ...
            for i in range(8):
                ...
                if next[0] >= 0 and next[1] >= 0 and next[0] < self.GRIDSIZE and next[1] < self.GRIDSIZE:
                        if self.grid[next[0]][next[1]] < 50:
                            if self.cost[current[0]][current[1]] + self.dCost[i] + abs(self.goal[0]-next[0]) + abs(self.goal[1]-next[1]) < self.cost[next[0]][next[1]]:
                                self.cost[next[0]][next[1]] = self.cost[current[0]][current[1]] + self.dCost[i] + abs(self.goal[0]-next[0]) + abs(self.goal[1]-next[1])
