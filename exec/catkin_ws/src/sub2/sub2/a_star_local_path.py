...
class astarLocalpath(Node):
    ...
    def path_callback(self,msg):
        ...
        self.is_path=True
        self.global_path_msg=msg.poses


    def timer_callback(self):
        if self.is_odom and self.is_path ==True:
            print('global path size : {}'.format(len(self.global_path_msg)))
            local_path_msg=Path()
            ...
            min_dis=float('inf')
            for i,waypoint in enumerate(self.global_path_msg) : 
            ...
            if current_waypoint != -1 : 
                if current_waypoint + self.local_path_size < len(self.global_path_msg):
                    for num in range(current_waypoint, current_waypoint + self.local_path_size):
                        tmp_pose = PoseStamped()
                        tmp_pose.pose.position.x = self.global_path_msg[num].pose.position.x
                        tmp_pose.pose.position.y = self.global_path_msg[num].pose.position.y
                        tmp_pose.pose.orientation.w = 1.0
                        local_path_msg.poses.append(tmp_pose)
                
                else :
                    for num in range(current_waypoint, len(self.global_path_msg)):
                        tmp_pose = PoseStamped()
                        tmp_pose.pose.position.x = self.global_path_msg[num].pose.position.x
                        tmp_pose.pose.position.y = self.global_path_msg[num].pose.position.y
                        tmp_pose.pose.orientation.w = 1.0
