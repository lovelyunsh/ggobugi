...
class Localization:
    ...
    def _weighting(self, points):
        ...
        for i in range(self.num_particle):
            ...
            particle_pose_x, particle_pose_y = self._points_in_map(particle_pose[0], particle_pose[1]) #파티클 하나의 포즈
            ...
            points_global_x, points_global_y = self._points_in_map(transformed_points[0, :], transformed_points[1, :]) #라이다 포인터

            
            #로직 11. weighting
            # 맵안에 존재하는 유효한 라이다 포인터들의 픽셀좌표만 남기고 그 픽셀 좌표 상에서의
            # map의 값만 골라오십시오
    ...
    def _resampling(self):
        ...
        for i in range(n_particles): #누적합
        ...
        #로직 12. resampling
        # np.random.uniform로 파티클 개수만큼 난수를 뽑은 다음,
        # 그 난수에 
        for i in range(n_particles): # n_파티클 만큼 만드는 작업
...
class Localizer(Node):
    ...
    def imu_callback(self,msg):
        ...
        if self.is_imu ==False :    
            ...
            _,_,self.odom_theta=imu_q.to_euler()
    ...
    def turtlebot_status_callback(self, msg):
        ...
        if self.is_init_pose ==True and self.is_imu==True:
            ...
            if self.is_status == False :
                ## 첫번째 스텝에서 is_status는 False로 시작하며 들어가고,
                ## prev_time만 save 하는 과정
                self.prev_time=rclpy.clock.Clock().now()

                ## prev_time가 기록되면 이 다음에 status msg에서 odometry를
                ## 받기 시작한다.
                self.is_status=True
                
                self.odom_x = msg.twist.angular.x
                self.odom_y = msg.twist.angular.y
