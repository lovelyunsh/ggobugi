...
# 제어 입력값은 Follow the carrot을 이용해 경로를 추종하는 각속도를 구하고, 선속도는 적절이 정해줍니다.
...
# 4. 로봇 주어진 경로점과 떨어진 거리(lateral_error)와 로봇의 선속도를 이용해 전방주시거리 설정
# 5. 전방 주시 포인트 설정
...
class followTheCarrot(Node):
    ...
    def timer_callback(self):
        ...
        if self.is_status and self.is_odom ==True and self.is_path==True:
            ...
            if len(self.path_msg.poses)> 1:
                ...
                전방주시거리는 임의로 정한다. (선속도와 lateral_error를 곱함)

                임의로 정한 전방주시거리(lfd)가 min_lfd과 가장 가깝다면 전방주시거리는 미리 설정해놓은 min_lfd로 대체한다.
                
                lfd가 max_lfd보다 길다면 전방주시거리는 미리 설정해놓은 max_lfd로 대체한다.

                적당히 전방주시 거리를 정해놓고 각각을 비례하게 만들어놓는다.

                '''
                # if lateral_error < 1:
                #     lateral_error = 1
                self.lfd = (self.odom_msg.twist.twist.linear.x + lateral_error) * 2.0 
                ...
                dis 는 로봇과 가장 가까운 점에서 전방주시포인트까지의 거리

                dis와 차이를 min_dis에 저장한다.
                
                forward_point 가 전방주시포인트이다.
                '''
                ...
                if self.is_look_forward_point :
                    ...
                    trans_matrix는 로봇좌표계에서 기준좌표계(Map)로 좌표변환을 하기위한 변환 행렬입니다. 로봇 -> 기준
                    det_tran_matrix는 trans_matrix의 역행렬로, 기준좌표계(Map)에서 로봇좌표계로 좌표변환을 하기위한 변환 행렬입니다.  기준 -> 로봇
                    ...
                    local_forward_point= np.matmul(det_trans_matrix, global_forward_point)
                    theta= -atan2(local_forward_point[1], local_forward_point[0])
                    # print(self.robot_yaw/3.14*180, theta/3.14*180)
    ...
    def odom_callback(self, msg):
        ...
        _,_,self.robot_yaw=Quaternion.to_euler(q)
