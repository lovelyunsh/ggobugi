...
class IMGParser(Node):
    ...
    def img_callback(self, msg):
        ...
        ## cv2 내의 이미지 색 채널 컨버터로 bgr 색상을 gray scale로 바꾸십시오.
        ...
        """
        img_resize = cv2.resize(img_gray,dsize=(640,480),interpolation=cv2.INTER_AREA)

        # 로직 5. 이미지 imshow
        
        # cv2.imshow("Sub1 skeleton",img_bgr)
        # cv2.imshow("Sub1 skeleton", img_gray)
        cv2.imshow("Sub1 skeleton", img_resize)
        cv2.waitKey(1)
