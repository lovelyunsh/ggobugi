...
def estimate_position(xyii, bbox):
    ...
    if bbox.shape[0]!=0:
        ...
        for i in range(bbox.shape[0]):
            ...
            #xyv = xyii[np.logical_and(xyii[:, 1]>=cy-0.4*h, xyii[:, 1]<cy+0.4*h), :]
            for ctr in zip(xyv[:, 0].astype(np.int32), xyv[:, 1].astype(np.int32)):
                image_process = cv2.circle(image_process, ctr, 2, (255,0,0),-1)
...
def visualize_images(image_out, t_cost):
    ...
    cv2.putText(image_out,'ResNet',(30,50), font, 1,(0,255,0), 2, 0)
...
def main(args=None):
    ...
    subscription_img = g_node.create_subscription(CompressedImage, '/image_jpeg/compressed', img_callback, 1)
    #publisher
    img_publisher = g_node.create_publisher(CompressedImage, '/person/img', 10)
    ...
    while rclpy.ok():
        ...
        for i in classes_pick[0] :
            if((int)(i) == 1):
                is_person = True
        if(is_person):
            img_msg.header.frame_id = 'find'
            img_msg.data = origin_img
        else :
            img_msg.header.frame_id = 'nofind'
            img_msg.data = origin_img
        ...
        if len(boxes_detect) != 0:
            ...
            #ostate_list = estimate_position(xyii,bbox)
            ostate_list = []
